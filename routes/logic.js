...
logic.get("/:name([A-Za-z]+)", async (req, res, next) => {
    const name = req.params.name;
    const rows = await bd.query("SELECT * FROM `empleados` WHERE `emp_name` = '" + name + "' ;");
...
logic.put("/:id([0-9]{1,3})", async (req, res, next)=>{
    ...
    if(emp_id && emp_name && emp_apellidos && emp_telefono && emp_correo && emp_direccion && emp_acc && emp_contrasenia){
        let query = `UPDATE empleados SET emp_name='${emp_name}', emp_apellidos=${emp_apellidos}, `;
        query += `emp_telefono=${emp_telefono}, emp_correo=${emp_correo}, emp_direccion=${emp_direccion}, emp_acc=${emp_acc}, emp_contrasenia${emp_contrasenia} WHERE emp_id = ${emp_id}`;
        const rows = await bd.query(query);
        return rows.affectedRows == 1? res.status(200).json({code:200, message: "Empleado actualizado correctamente"}) :res.status(500).json({code: 500, message: "Ocurrio un problema"});
    }
    return res.status(500).json({code: 500, message: "Campos incompletos"})
});

logic.delete("/:id([0-9]{1,3})",async (req, res, next)=>{
    const query = `DELETE FROM empleados WHERE emp_id=${req.params.emp_id}`;
    const rows = await bd.query(query);
